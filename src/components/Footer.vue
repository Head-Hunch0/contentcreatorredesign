<template>
    <footer class="bg-neutral-50 py-16 lg:py-24">
        <div class="max-w-7xl mx-auto px-4 xl:px-0 flex flex-col gap-y-12">
            <div class="grid gap-y-6 sm:grid-cols-2 sm:gap-x-8 md:px-4 lg:grid-cols-4 lg:gap-x-12 lg:gap-y-0 lg:px-8">

                <!-- Navigation Sections -->
                <div v-for="section in footerSections" :key="section.title" class="flex flex-col items-start gap-y-4">
                    <div class="text-sm font-bold text-neutral-700">{{ section.title }}</div>
                    <div class="flex flex-col items-start gap-y-1.5">
                        <template v-for="link in section.links" :key="link.text">
                            <div v-if="link.isWrapper" class="flex flex-wrap items-center gap-1">
                                <a :href="link.href" :title="link.title"
                                    class="whitespace-nowrap text-sm font-medium text-neutral-500" :class="link.class">
                                    {{ link.text }}
                                </a>
                                <div v-if="link.badge"
                                    class="items-center justify-center rounded-full text-sm font-medium whitespace-nowrap shadow-[0_2px_10px_0px_rgba(0,0,0,0.15)] inline-flex px-2 py-0.5"
                                    :class="link.badgeClass">
                                    {{ link.badge }}
                                </div>
                            </div>
                            <a v-else :href="link.href" :title="link.title"
                                class="whitespace-nowrap text-sm font-medium text-neutral-500" :class="link.class">
                                {{ link.text }}
                            </a>
                        </template>
            ...
            <div class="border-b border-b-neutral-200"></div>

            <div class="flex flex-wrap items-center justify-between gap-4 md:px-4 lg:px-8">
                ...
                <div class="flex flex-wrap items-center gap-6">
                    <div class="flex items-center gap-x-6">
                        <a v-for="link in footerLinks" :key="link.text" :href="link.href" :title="link.title"
                            class="whitespace-nowrap text-sm font-medium text-neutral-500 transition hover:text-neutral-700">
                            {{ link.text }}
                        </a>
                    </div>

                    <div class="flex items-center gap-x-4">
                        <div class="mr-2 h-4 w-[1px] bg-neutral-200"></div>
                        <div class="flex flex-wrap items-center gap-4">
                            <a v-for="social in socialLinks" :key="social.name" :href="social.url" :title="social.name"
                                ...
                                <svg class="h-5 text-slate-700" viewBox="0 0 16 16" fill="none"
                                    xmlns="http://www.w3.org/2000/svg">
                                    <path fill-rule="evenodd" clip-rule="evenodd" :d="social.path"
                                        fill="currentColor" />
                                </svg>
                            </a>
                        </div>
...
const copyrightText = ref('PromptEdit.com. All rights reserved.')
...
const footerSections = ref<FooterSection[]>([
    ...
    },
    {
        ...
        links: [
            ...
            {
                text: 'Save on Credits',
                href: '#',
                title: 'Save on Credits',
                isWrapper: true,
            }
        ]
    },
